...
>> É legal ter ao menos um plot mostrando todos os pontos, além de sumários como o boxplot e outros.
...
dados %>%
    ...
    geom_jitter(width = .25, size = .3, alpha = .5) + 
    stat_summary(fun.y="mean", geom="point", size=3,  
                 position=position_dodge(width=0.75), color="pink")+
...
>> Usa um termo mais técnico do que espaçamento dos dados. E qual foi a medida que você considerou? Amplitude? IQR? Desvio padrão?
...
```
  ...
  >> Gostei da análise de início/fim de séries, mas não houve a de temporadas, correto? 